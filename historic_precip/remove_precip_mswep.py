...
swaths_path = '/nfs/turbo/seas-hydro/laratt/TCdata/precip_swaths/mswep/' # paths to TC swaths
...
if __name__ == '__main__':
    ...
    if PARALLEL:
        ...
        cluster = LocalCluster(n_workers=4, threads_per_worker=1, memory_limit='15GB')
    ...
    if COMBINE_FILES:
        ...
        if COMBINE_FILES == True:
            ...
            """# Function to combine datasets using combine_first in parallel
            data_directory = '/nfs/turbo/seas-hydro/laratt/TCdata/precip_swaths/mswep'
            datasets = [xr.open_dataset(os.path.join(data_directory, filename), chunks={'time': 100}) 
                        for filename in os.listdir(data_directory) if filename.endswith('.nc')]
            ...
            # Apply the combine_datasets function lazily with Dask
            combined_ds = dask.delayed(combine_datasets)(datasets)

            # Function to save combined dataset as Zarr
            def save_combined_ds_as_zarr(combined_ds, output_path):
                # Compute the final result from Dask (this will trigger the actual computation)
                final_combined_ds = combined_ds.compute()
                
                # Write the result to a Zarr file (compressed and chunked)
                final_combined_ds.to_zarr(f'{output_path}/combined_precip.zarr', mode='w', consolidated=True)

            # Monitor progress and execute the computation
            with ProgressBar():
                dask.compute(save_combined_ds_as_zarr(combined_ds, output_path))"""
            
            """data_directory = '/nfs/turbo/seas-hydro/laratt/TCdata/precip_swaths/mswep'

            # Use delayed to read the datasets lazily
            def load_dataset(filename):
                return xr.open_dataset(os.path.join(data_directory, filename), chunks={'time': 1000})  # Adjust chunk size

            datasets = [
                delayed(load_dataset)(filename) for filename in os.listdir(data_directory) if filename.endswith('.nc')
            ]

            # Function to combine datasets lazily
            def combine_datasets(datasets):
                combined_ds = datasets[0]
                for ds in datasets[1:]:
                    combined_ds = combined_ds.combine_first(ds)
            ...
            # Apply the combine_datasets function
            combined_ds = delayed(combine_datasets)(datasets)

            # Save combined dataset in Zarr format
            def save_combined_ds_as_zarr(combined_ds, output_path):
                final_combined_ds = combined_ds.compute()  # Trigger the computation
                final_combined_ds.to_zarr(f'{output_path}/combined_precip.zarr', mode='w')  # Save as Zarr
            
            save_combined_ds_as_zarr(combined_ds, output_path)"""
        else:
    ...
    if REMOVE_TCS:
        ...
        tc_times = [f'{yr}{day}' for yr,day in zip(pd.to_datetime(combined_ds.time.values[0]).year,pd.to_datetime(combined_ds.time.values[0]).dayofyear)]
        # loop over times that TCs are active
        for t in tc_times:
            logger.info(f'Processing time {t}')
            # open IMERG file(s) for time TC is active
            fp = f'{imerg_daily_path}/{t}*'
            ...
            result.to_netcdf(f'{output_path}/{t}.TCREMOVED.nc')
