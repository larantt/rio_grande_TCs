...
# filepaths to data
IMERG_PATH = "/nfs/turbo/seas-hydro/laratt/mswep/"
IMERG_EXTN = '-S000000-E235959.V07B.nc4.nc4?precipitation[0:0][449:1100][1019:1349],time,lon[449:1100],lat[1019:1349]'
...
SWATH_OUTPATH = "/nfs/turbo/seas-hydro/laratt/TCdata/precip_swaths/mswep"
...
SAVE_CLEAN_TRACKS = True   # bool - should the cleaned EBT data be saved to disk as a csv? False if TRACKS_FROM_DISK specified

# globals for swath extraction
SOURCE = 'MSWEP'        # precipitation product being used
...
START_YR = 1979            # year to start data extraction
END_YR = 2020              # year to end data extraction
STORM_ID = None            # run test for individual TC - use the ATCF ID from the EBTRAK dataset (e.g. 'AL012010' for Alex)
...
# Extract the year and day-of-year directly using pandas .dt accessor
database['year'] = database['date'].dt.year
database['day_of_year'] = database['date'].dt.dayofyear

# Combine the year and day-of-year into the format YYYYDDD (e.g. 1979126)
database['day_of_year_str'] = database['year'].astype(str) + database['day_of_year'].apply(lambda x: f"{x:03d}")

# Now generate the file paths all at once
database['file_path'] = '/nfs/turbo/seas-hydro/laratt/mswep/' + database['day_of_year_str'] + '.nc'

# The result is a DataFrame with the corresponding file paths
imerg_files = database['file_path'].tolist()
...
precip_ds = xr.open_mfdataset(imerg_files,decode_times=False,decode_cf=False,engine='netcdf4')
#precip_ds = xr.open_mfdataset(imerg_files,decode_times=False,decode_cf=False,engine='netcdf4',parallel=True)
precip_ds = xr.decode_cf(precip_ds)
...
for track in database.track_id.unique():
    ...
    centres = list(zip(storm['lon'], storm['lat']))
    ...
    ds = md.daily_radial_precip(centres, times, precip_ds, 
